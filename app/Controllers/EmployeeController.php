...
class EmployeeController extends BaseController
{
    ...
    public function check_approved_stamp()
    {
        $user = $this->user->find(session()->user_id);

        $stamp = $this->stamp
            ->where('stamp_type', 'Approved Stamp')
            ->where('stamp_status', 1)
            ->first();

        if (!$stamp) {
            $response['success'] = false;
            $response['message'] = 'Approved stamps have not been set up or assigned to you';
            return $this->response->setJSON($response);
        }

        $stamp_users = json_decode($stamp['stamp_users']);
        if (!in_array(session()->user_id, $stamp_users)) {
            $response['success'] = false;
            $response['message'] = 'Approved stamps have not been set up or assigned to you';
            return $this->response->setJSON($response);
        }
        ...
        $response['success'] = true;
        $response['message'] = 'Approved stamp found.';
        return $this->response->setJSON($response);
    }
